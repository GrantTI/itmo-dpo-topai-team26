...
**Необходимые условия:** среда Python 3.10+, Docker (по заданиям этапов), редактор кода; исходные требования к ПО — в разделе «Техническая база».
...
- Допускается использование официальной документации FastAPI, Python и связанных библиотек, а также учебных материалов дисциплины.
- Допускается применение генеративного ИИ для справок и подготовки черновиков кода при условии, что обучающийся понимает архитектуру решения и способен воспроизвести и пояснить ключевые фрагменты на защите.
- Не допускается представление непроверенного сгенерированного кода без возможности объяснить назначение компонентов (маршруты, контроллеры, сервисы, обработку ошибок).
- Секреты, ключи доступа и персональные данные в репозиторий и отчётные материалы не включаются.
