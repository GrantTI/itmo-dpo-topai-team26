# Лабораторная работа 
## Тема: Визуализация данных в ML моделях. Работа с HTML и CSS
...
**1. Какие теги используются для создания заголовков в HTML? Какой из них самый важный?**

Ожидаемый ответ: `<h1>` до `<h6>`, самый важный — `<h1>`.

**2. В чем разница между тегами `<ul>` и `<ol>`?**

Ожидаемый ответ: `<ul>` создает маркированный (ненумерованный) список, `<ol>` — нумерованный.

**3. Как создать ссылку, которая открывается в новой вкладке?**

Ожидаемый ответ: Использовать атрибут `target="_blank"` в теге `<a>`.

**4. Для чего используется атрибут `alt` у изображений?**

Ожидаемый ответ: Для описания изображения (отображается при его отсутствии и используется для доступности).

**5. Какие типы полей ввода существуют в HTML? Назовите минимум 5.**

Ожидаемый ответ: text, email, password, checkbox, radio, file, number, date и др.

---

## Вопросы средней сложности

**6. Как объединить радиокнопки в группу, чтобы можно было выбрать только один вариант?**

Ожидаемый ответ: Задать одинаковое значение атрибута `name` для всех радиокнопок в группе.

**7. В чем отличие между `margin` и `padding` в CSS?**

Ожидаемый ответ: `margin` — внешний отступ (вне границы элемента), `padding` — внутренний отступ (внутри границы, между границей и контентом).

**8. Как с помощью CSS изменить цвет рамки поля ввода при получении фокуса?**

Ожидаемый ответ: Использовать псевдокласс `:focus`, например, `input:focus { border-color: blue; }`.

**9. Как изменить цвет текста-подсказки (плейсхолдера) в поле ввода?**

Ожидаемый ответ: Использовать псевдоэлемент `::placeholder` в CSS.

**10. Как создать кастомные чекбоксы с помощью CSS?**

Ожидаемый ответ: Скрыть стандартный чекбокс (`display: none` или `opacity: 0`) и стилизовать псевдоэлементы (`::before`, `::after`) или использовать свойство `accent-color` для простых изменений.

---

## Дополнительные вопросы (для продвинутых)

**11. Что такое валидация форм? Какие атрибуты HTML5 помогают в валидации?**

Ожидаемый ответ: Валидация — проверка введенных данных. Атрибуты: `required`, `pattern`, `min`, `max`, `type="email"`, `type="url"` и др.

**12. Как с помощью CSS сделать форму адаптивной (responsive)?**

Ожидаемый ответ: Использовать относительные единицы (%, em, rem), медиа-запросы (`@media`), flexbox или grid.

**13. Какие псевдоклассы CSS вы знаете и для чего они используются?**

Ожидаемый ответ: `:hover` (наведение), `:focus` (фокус), `:active` (активное состояние), `:first-child` (первый дочерний), `:nth-child()` (n-й дочерний) и др.

**14. Какие существуют способы стилизации чекбоксов и радиокнопок?**

Ожидаемый ответ: Использование свойства `accent-color` (простой способ) или полная кастомизация с помощью скрытия стандартного элемента и использования псевдоэлементов `::before` и `::after`.

**15. Как можно улучшить доступность (accessibility) формы?**

Ожидаемый ответ: Использовать теги `<label>` с атрибутом `for`, добавлять `aria-*` атрибуты, обеспечивать достаточный контраст текста, использовать семантические теги.
