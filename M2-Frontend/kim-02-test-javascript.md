...
- `children` — возвращает только HTML-элементы (без текстовых узлов и комментариев) в виде `HTMLCollection`;
- `childNodes` — возвращает все узлы, включая текстовые и комментарии, в виде `NodeList`;
- `querySelectorAll('*')` — возвращает все элементы внутри в виде статического `NodeList`.
</details>
...
**Вопрос 25.** Какое значение вернет `null == undefined`?
- A) `true`
- B) `false`
- C) `undefined`
- D) `null`
...
<details>
<summary>Показать ответ</summary>
**Правильный ответ: A)** При нестрогом сравнении (`==`) `null` и `undefined` считаются равными.
</details>

---

**Вопрос 26.** Какое значение вернет `null === undefined`?
- A) `true`
- B) `false`
- C) `undefined`
- D) `null`
...
<details>
<summary>Показать ответ</summary>
**Правильный ответ: B)** При строгом сравнении (`===`) `null` и `undefined` имеют разные типы, поэтому возвращается `false`.
</details>
...
# Критерии оценки теста по JavaScript

| Балл | Критерий |
|:----:|----------|
| 3 | Правильные ответы на 25–30 вопросов (83–100%) — отличное владение основами JavaScript, включая работу с DOM-элементами, событиями, методами консоли, типами данных и операторами сравнения |
| 2 | Правильные ответы на 18–24 вопроса (60–82%) — хорошее понимание базовых концепций JavaScript, допустимы отдельные ошибки в специфических вопросах (типы данных, строгое/нестрогое сравнение) |
| 1 | Правильные ответы на 10–17 вопросов (33–59%) — удовлетворительное знание материала, требуется дополнительное изучение работы с DOM и событийной моделью |
| 0 | Правильные ответы на 0–9 вопросов (0–32%) — материал не усвоен, требуется повторное изучение основ JavaScript |
| **3** | **ИТОГО** |
